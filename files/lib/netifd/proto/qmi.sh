...
proto_qmi_setup() {
	...
	while uqmi -s -d "$device" --get-pin-status -t 5000 | grep '"UIM uninitialized"' > /dev/null; do
		[ -e "$device" ] || return 1
	...
	if uqmi -s -d "$device" --get-pin-status -t 5000 | grep '"Not supported"\|"Invalid QMI command"' > /dev/null; then
		[ -n "$pincode" ] && {
		...
		json_load "$(uqmi -s -d "$device" --get-pin-status -t 5000)"
